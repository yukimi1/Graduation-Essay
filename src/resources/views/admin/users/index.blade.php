...
<div class="col-2 ms-auto mb-4">
    <form action="{{ route('admin.users') }}" method="get">
        <input type="text" name="search" value="{{ $search }}" placeholder="Search for names..." class="form-control form-control-sm">
    </form>
</div>
<table class="table border table-hover align-middle text-secondary">
    <thead class="table-success small text-muted text-uppercase">
        <th></th>
        <th>Name</th>
        <th>Email</th>
        <th>Created At</th>
    ...
    <tbody>
        @forelse($all_users as $user)
            <tr>
                <td>
                    @if($user->avatar)
                        <img src="https://graduationessay.s3-ap-northeast-1.amazonaws.com/{{ $user->avatar }}" alt="" class="rounded-circle avatar-md d-block mx-auto">
                    @else
                        <i class="fa-solid fa-circle-user text-secondary icon-admin d-block text-center"></i>
                    @endif
                </td>
                <td>
